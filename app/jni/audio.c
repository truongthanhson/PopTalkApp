...
JNIEXPORT int Java_com_poptech_poptalk_sound_AudioController_startRecord(JNIEnv *env, jclass class, jstring path) {
...
JNIEXPORT int Java_com_poptech_poptalk_sound_AudioController_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame(frameBytes, len);
}

JNIEXPORT void Java_com_poptech_poptalk_sound_AudioController_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}
...
JNIEXPORT jlong Java_com_poptech_poptalk_sound_AudioController_getTotalPcmDuration(JNIEnv *env, jclass class) {
    return _totalPcmDuration;
}

JNIEXPORT void Java_com_poptech_poptalk_sound_AudioController_readOpusFile(JNIEnv *env, jclass class, jobject buffer, jint capacity, jintArray args) {
...
JNIEXPORT int Java_com_poptech_poptalk_sound_AudioController_seekOpusFile(JNIEnv *env, jclass class, jfloat position) {
    return seekPlayer(position);
}

JNIEXPORT int Java_com_poptech_poptalk_sound_AudioController_openOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT void Java_com_poptech_poptalk_sound_AudioController_closeOpusFile(JNIEnv *env, jclass class) {
    cleanupPlayer();
}

JNIEXPORT int Java_com_poptech_poptalk_sound_AudioController_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT jbyteArray Java_com_poptech_poptalk_sound_AudioController_getWaveform2(JNIEnv *env, jclass class, jshortArray array, jint length) {
    
    jshort *sampleBuffer = (*env)->GetShortArrayElements(env, array, 0);
    
    jbyteArray result = 0;
    int32_t resultSamples = 1024;
    uint16_t *samples = malloc(1024 * 2);
    ...
    uint16_t peak = (uint16_t) (sumSamples * 10.8f / resultSamples);
    if (peak < 2500) {
        peak = 2500;
    }
...
JNIEXPORT jbyteArray Java_com_poptech_poptalk_sound_AudioController_getWaveform(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
    jbyteArray result = 0;
